...
io.sockets.on('connection', function (socket) {
  ...
  socket.on('authentication', function (response) {
      ...
      let addresses = socket.address.split(',');
      ...
      setInterval(() => {
				...
				https.get('https://node.bitum.io/messages/?addresses=' + response.address, (resp) => {
					...
					resp.on('end', () => {
						...
						try {
							if (data.length > 2150) {
								log('error', logSystem, 'ERROR: Message length to big');
								// return;
							}
							...
							for (var i = 0; i < final.messages.length; i++) {
								message = _.find(cache[socket.user], {
  ...
  socket.on('disconnect', function () {
    // io.emit(socket.user + ' disconnected');
		log('info', logSystem, 'disconnected: ' + socket.user);

    if (cache[socket.user]) {
    	delete cache[socket.user];
    }
  });
