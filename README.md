...
## Encrypted messenger with unfalsifiable transactions
  ...
  ```
    # With service
    systemctrl start bitum-node.service
    
    # With bin
    cd /opt/
    src/bin/bitum-node --rpc-bind-port=8087 --p2p-bind-port=3031 --network=MAINNET --prefix=bitum --rpc-bind-ip=IP --p2p-bind-ip=IP
    
  ```

  3 - Monitoring
  ```
    journalctrl -u bitum-node.service -f
  ```
...
```
where:
* `<ip>` is IPv4 address of `bitum-node` service. 
* `<port>` is TCP port of `bitum-node`. By default the service is bound to `8087`.
...
```
    http://<ip>:<port>/status # https://node.bitum.io/status
```
...
```
    http://<ip>:<port>/top-block # https://node.bitum.io/top-block
```
...
```
    http://<ip>:<port>/top-height # https://node.bitum.io/top-height
```
...
```
    http://<ip>:<port>/public-key # https://node.bitum.io/public-key
```
...
```
    http://<ip>:<port>/mempool # https://node.bitum.io/mempool
```
...
```
    http://<ip>:<port>/peers # https://node.bitum.io/peers
```
...
```
    curl -X POST \
       https://node.bitum.io/get-blocks \
...
```
     curl -X POST \
       https://node.bitum.io/get-block-by-height \
       -H 'Content-Type: application/json' \
       -d '{
       "blockHeight": 45739
     }'
...
```
    curl -X POST \
      https://node.bitum.io/get-block-by-hash \
      -H 'Content-Type: application/json' \
      -d '{
...
```
    curl -X POST \
      https://node.bitum.io/get-transaction-by-hash \
      -H 'Content-Type: application/json' \
      -d '{
...
```
    curl -X POST \
      https://node.bitum.io/get-transfer-by-hash \
      -H 'Content-Type: application/json' \
      -d '{
...
```
    curl -X POST \
      https://node.bitum.io/get-wallet-address-infos \
      -H 'Content-Type: application/json' \
      -d '{
...
```
    curl -X POST \
      https://node.bitum.io/get-wallet-addresses-infos \
      -H 'Content-Type: application/json' \
      -d '{
...
```
curl 'https://node.bitum.io/transaction' \
  -H 'Content-Type: application/json' \
...
```
    curl -X POST \
      https://node.bitum.io/getBlockTemplate \
      -H 'Content-Type: application/json' \
      -d '{
...
```
    curl -X POST \
      https://node.bitum.io/submitBlockHash \
      -H 'Content-Type: application/json' \
      -d '{
...
## Export
Export all your bitum blockchain into gz file:
```
  bin/bitum-export --prefix=bitum --force=true
  # Output file => ./bitum.tar.gz
```

## Import
Import blockchain from gz file:
```
  bin/bitum-import --file=bitum.tar.gz --prefix=abcd --force=true
```

#### Cut or reset your blockchain
For any reason you can reset or cut your blockchain from height (0 for all)
```
  bin/bitum-reset --prefix=abcd --height=777
```

# Docker - Dev environment

  1. Create bitum folder
  ```
    mkdir bitum && cd bitum
  ```

  2. Clone projects
  ```
    git clone git@github.com:bitum-project/bitum-blockchain.git
    git clone git@github.com:bitum-project/bitum-wallet.git
    git clone git@github.com:bitum-project/bitum-explorer.git
    git clone git@github.com:bitum-project/bitum-website-viewer.git
  ...
  5. Run bitum-node container
  ```
    docker exec -it bitum-node bash
  ...
  ```
    sed -i 's/198\.199\.73\.197/0\.0\.0\.0/g' /opt/src/bin/bitum-node
  ```

  8. Start bitum-node daemon
  ```
    cd /opt/
    src/bin/bitum-node --rpc-bind-port=8087 --p2p-bind-port=3031 --network=MAINNET --prefix=bitum
  ```

  8. Start other bitum-node daemon into other terminal
  ```
    docker exec -it bitum-node bash

    # root@bitum-node:/#
    cd /opt/
    src/bin/bitum-node --rpc-bind-port=8087 --p2p-bind-port=3031 --network=MAINNET --prefix=alice
  ```
  ...
  ```
    ...
    # root@bitum-node:/#
    cd /opt/src/
    bin/bitum-miner --wallet-address=0x5967a4016501465CD951a1e3984F772AfDeB5207 --rpc-port=8087 --rpc-ip=0.0.0.0
  ```

  10. Run bitum explorer http://localhost:8000
  ```
    docker exec -it bitum-explorer-phpfpm bash
  ...
  ```
    ...
    11. Run bitum website viewer http://localhost:8001
  ```
    docker exec -it bitum-website-viewer-phpfpm bash
  ...
  12. Replace remote node ip by bitum-node (container name)
  ```
    sed -i 's#https:\/\/node.bitum.io\/#http:\/\/bitum-node:8087\/#g' /www/src/app/App.php
  ```
