...
class P2pServer
{
        ...
        $connection->on('data', function (string $data) use ($connection): void {
            ...
            if ($packet->isValid()) {
                switch ($packet->getType()) {
                    ...
                    case Packet::HELLO_BITUM:
                        $this->handleHelloBitum($packet, $connection);
                        break;

                    case Packet::HELLO_BITUM_RESPONSE:
                        $this->handleHelloBitumResponse($packet, $connection);
                        break;
    ...
    public function handleHelloBitum($packet, ConnectionInterface $connection)
    {
        ...
        $connection->write(Packet::prepare(
            $this->network,
            Packet::HELLO_BITUM_RESPONSE,
            $this->getHelloBitumMessage(),
            $this->getPublicKey()
        ));
    }

    public function handleHelloBitumResponse($packet, ConnectionInterface $connection)
    {
    ...
    private function getHelloBitumMessage() {
        return [
